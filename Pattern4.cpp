...
int main(int argc, char *argv[])
{
    ...
    Ultrahaptics::Vector3 position1((-li.length / 2) + (3 * Ultrahaptics::Units::centimetres), (-li.length / 2) + (li.length), distance);
    Ultrahaptics::Vector3 position2((-li.length / 2) - (1.75 * Ultrahaptics::Units::centimetres) + (3 * Ultrahaptics::Units::centimetres) ,
    ...
    for(;;){
        ...
        if(iterations%resolution == 0 && iterations != 0){
            emitter.stop();
            std::this_thread::sleep_for(std::chrono::milliseconds(wait_time_ms*20));
            counter += 1;
            if(counter == 3){
                return 0;
            }
