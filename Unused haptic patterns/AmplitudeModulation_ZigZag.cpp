...
struct Line
{
    double length = 60.0 * Ultrahaptics::Units::mm;
    double width = 5 * Ultrahaptics::Units::mm;
    double period = 1;
};
...
int main(int argc, char *argv[])
{
    ...
    float distance = 10.0 * Ultrahaptics::Units::centimetres;
    ...
    Side_Index side = RIGHT;

    Ultrahaptics::Vector3 position1((-sqr.length / 2), (-sqr.length / 2)+sqr.length, distance);
    ...
    //1200
    auto wait_time_ms = (int) (2500 * 2 * sqr.period / (4 * side_resolution));
    ...
    for(;;){
        ...
        if(side == RIGHT) {
            position1.x += sqr.width / side_resolution;
